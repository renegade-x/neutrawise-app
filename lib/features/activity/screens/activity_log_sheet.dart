...
class _ActivityLogSheetState extends ConsumerState<ActivityLogSheet> {
  ...
  void initState() {
    ...
    if (widget.existingLog != null) {
      ...
      _energyConfirmed = widget.existingLog!.energyCo2 > 0 || _energyDeviations.isNotEmpty; 
  ...
  void _submitLog() async {
    ...
    try {
      ...
      final profile = await ref.read(userRepositoryProvider).getUserProfile(user!.id);
      ...
      final savedDelta = log.co2SavedVsBaseline - (widget.existingLog?.co2SavedVsBaseline ?? 0.0);
      final isFirstLogOfDay = widget.existingLog == null;
      
      final updatedProfile = profile.copyWith(
        xp: (profile.xp ?? 0) + xpDelta,
        currentStreak: isFirstLogOfDay ? (profile.currentStreak ?? 0) + 1 : profile.currentStreak,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Log saved! +${log.xpEarned} XP', style: const TextStyle(color: Colors.white)),
      ...
      child: DefaultTabController(
        ...
        child: Column(
          children: [
            ...
            const Text('Log Activity', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const TabBar(
              tabs: [Tab(icon: Icon(Icons.directions_car)), Tab(icon: Icon(Icons.restaurant)), Tab(icon: Icon(Icons.bolt))],
      ...
      child: Column(
        ...
        children: [
          DropdownButtonFormField<String>(
            ...
            items: ['car', 'ev', 'motorcycle', 'bus', 'train', 'metro', 'bicycle', 'walking'].map((e) => DropdownMenuItem(value: e, child: Text(e.toUpperCase()))).toList(),
          ...
          ElevatedButton.icon(
            ...
            onPressed: () {
              ...
              if (dist != null && dist > 0) {
                setState(() {
                  _transportEntries.add(TransportEntry(mode: _transportMode, distanceKm: dist));
                  _distanceCtrl.clear();
                });
              }
            },
          ...
          const Text('Today\'s Trips:', style: TextStyle(fontWeight: FontWeight.bold)),
          ..._transportEntries.map((e) => ListTile(
      ...
      child: Column(
        ...
        children: [
          ...
          Autocomplete<OpenFoodFactsProduct>(
            ...
            fieldViewBuilder: (context, controller, focusNode, onFieldSubmitted) {
              ...
              return TextFormField(
                ...
                focusNode: focusNode,
                decoration: const InputDecoration(labelText: 'Food Name (Search)', border: OutlineInputBorder()),
              );
            },
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          subtitle: Text(option.brand ?? '', style: TextStyle(color: Colors.grey[400])),
          ...
          DropdownButtonFormField<String>(
            ...
            items: EmissionFactors.foodCategoryFactors.keys.map((e) => DropdownMenuItem(value: e, child: Text(e.replaceAll('_', ' ').toUpperCase()))).toList(),
          ...
          TextFormField(
            ...
            keyboardType: TextInputType.number,
            decoration: const InputDecoration(labelText: 'Amount (grams)', border: OutlineInputBorder()),
          ),
          const SizedBox(height: 16),
          ElevatedButton.icon(
            icon: const Icon(Icons.add),
  ...
  Widget _buildEnergyTab() {
    final devOptions = ['more_than_usual', 'less_than_usual', 'no_ac', 'cold_showers', 'unplugged_devices'];
    return SingleChildScrollView(
      padding: const EdgeInsets.all(24),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
            ...
            children: devOptions.map((opt) {
              ...
              return Padding(
                ...
                child: CheckboxListTile(
                  ...
                  onChanged: (selected) {
                    setState(() {
                      if (selected == true) _energyDeviations.add(opt);
                      else _energyDeviations.remove(opt);
                    });
                  },
                ),
              );
