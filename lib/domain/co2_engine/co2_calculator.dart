...
class CO2Calculator {
  ...
  static DailyLog processDailyLog(
    ...
    for (var entry in transportEntries) {
      double factor = 0.0;
      if (entry.mode == 'car' || entry.mode == 'ev' || entry.mode == 'motorcycle') {
    ...
    for (var entry in foodEntries) {
      ...
      if (entry.co2Per100g != null && entry.co2Per100g! > 0) {
        ...
        final factorKgPerKg = EmissionFactors.foodCategoryFactors[entry.category] ?? 0.4;
    ...
    if (energyConfirmed) {
      ...
      for (var deviation in energyDeviations) {
        if (deviation == 'more_than_usual') deltaKwh += 2.0;
        else if (deviation == 'less_than_usual') deltaKwh -= 2.0;
        else if (deviation == 'no_ac') deltaKwh -= 5.0;
        else if (deviation == 'cold_showers') deltaKwh -= 1.5;
        else if (deviation == 'unplugged_devices') deltaKwh -= 0.5;
        else if (deviation == 'solar_panels_sunny') deltaKwh -= 4.0;
        else if (deviation == 'solar_panels_cloudy') deltaKwh -= 1.5;
      }
      ...
      final gridIntensity = profile.gridIntensity ?? EmissionFactors.gridIntensityPK;
      final electricityCO2 = totalKwh * gridIntensity;
      
      totalEnergyCo2 = electricityCO2 + (profile.dailyHeatingBaselineCo2 ?? 0.0);
    }
    ...
    // 5. XP Calculation
    int baseXP = 0;
    if (energyConfirmed && transportEntries.isNotEmpty && foodEntries.isNotEmpty) {
      baseXP = 50; 
    } else if (transportEntries.isNotEmpty || foodEntries.isNotEmpty || energyConfirmed) {
      baseXP = 20; 
    }
    ...
    if (xpEarned > 0) {
       if (percent >= 30) xpEarned += 25;
       else if (percent >= 15) xpEarned += 15;
       else if (percent >= 5) xpEarned += 8;
    }
