...
class OpenFoodFactsProduct {
  ...
  factory OpenFoodFactsProduct.fromJson(Map<String, dynamic> json) {
    ...
    double? co2;
    if (json['ecoscore_data'] != null && json['ecoscore_data']['agribalyse'] != null) {
      co2 = (json['ecoscore_data']['agribalyse']['co2_total'] as num?)?.toDouble();
    }
    ...
    return OpenFoodFactsProduct(
      ...
      co2Total: co2,
      fallbackCategory: EmissionFactors.mapOFFCategoryToFactorKey(json['categories'] ?? ''),
    );
  }
}
...
class OpenFoodFactsService {
  ...
  Future<List<OpenFoodFactsProduct>> searchFood(String query) async {
    ...
    try {
      final uri = Uri.parse('$_baseUrl?search_terms=$query&search_simple=1&action=process&json=1&page_size=10');
      final response = await http.get(
        uri,
        headers: {
          'User-Agent': 'NeutraWiseApp/1.0',
        },
      );
